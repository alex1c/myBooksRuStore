...
export const ocrCopy = {
	...
	privacy:
		'Фото и распознанный текст обрабатываются на устройстве и не загружаются на сервер.',
	privacyModelHint:
		'При первом использовании может потребоваться интернет для загрузки модели распознавания.',
...
export const moreCopy = {
	...
	importData: 'Импорт',
	importDataHint: 'Загрузить книги из CSV или Goodreads',
	reminders: 'Напоминания',
	remindersHint: 'Локальные напоминания о времени чтения',
	archive: 'Архив книг',
	...
	aboutBody:
		'Личный offline-first дневник чтения. Основные действия доступны без регистрации и без интернета. Для улучшения продукта используются технические события AppMetrica (без названий книг, цитат и заметок). Реклама Yandex Mobile Ads показывается вне активного чтения и не блокирует основные действия.',
} as const
...
export const remindersCopy = {
	title: 'Напоминания',
	subtitle: 'Спокойные локальные напоминания — без давления и без облака.',
	privacy:
		'Напоминания создаются на устройстве. Push-токены и сервер не используются.',
	loading: 'Загрузка…',
	enableLabel: 'Напоминать о чтении',
	timeLabel: 'Время',
	timeMinus: 'Уменьшить время на 15 минут',
	timePlus: 'Увеличить время на 15 минут',
	daysLabel: 'Дни',
	save: 'Сохранить',
	savedEnabled: 'Напоминания обновлены',
	savedDisabled: 'Напоминания отключены',
	errorTitle: 'Ошибка',
	permissionExplainTitle: 'Уведомления',
	permissionExplain:
		'Разрешите уведомления, чтобы приложение могло напоминать о выбранном времени чтения.',
	continueEnable: 'Продолжить',
	permissionDeniedTitle: 'Уведомления отключены для приложения.',
...
export const importCopy = {
	title: 'Импорт книг',
	subtitle: 'Добавьте книги из CSV без потери текущей библиотеки.',
	privacy: 'Файл обрабатывается только на устройстве.',
	supportsOur: '• CSV «Дневника чтения» (экспорт приложения)',
	supportsGoodreads: '• Экспорт Goodreads',
	supportsGeneric: '• Универсальный CSV с сопоставлением колонок',
	notBackup:
		'Импорт добавляет книги. Для полной замены данных используйте восстановление из резервной копии.',
	pickCsv: 'Импортировать CSV',
	parsing: 'Обрабатываем файл…',
	importing: 'Импортируем книги…',
	parseFailed: 'Не удалось прочитать CSV. Проверьте формат файла.',
	importFailed: 'Не удалось выполнить импорт. Изменения не сохранены.',
	errorTitle: 'Ошибка',
	titleMappingRequired: 'Укажите колонку с названием книги.',
	nothingSelected: 'Нет выбранных книг для импорта.',
	mappingTitle: 'Сопоставление колонок',
	mappingHint: 'Выберите, какая колонка соответствует каждому полю.',
	sampleHint: 'Пример строк после сопоставления:',
	continuePreview: 'К предпросмотру',
	previewTitle: 'Предпросмотр импорта',
	found: (n: number) => `Найдено: ${n}`,
	ready: (n: number) => `Готово к импорту: ${n}`,
	duplicates: (n: number) => `Возможные дубли: ${n}`,
	errors: (n: number) => `С ошибками: ${n}`,
	selected: (n: number) => `Выбрано: ${n}`,
	duplicatePolicy: 'Политика дублей',
	policySkip: 'Пропустить дубли',
	policyAdd: 'Добавить как другое издание',
	filterAll: 'Все',
	filterReady: 'Готово',
	filterDupes: 'Дубли',
	filterErrors: 'Ошибки',
	...
	badgeDupe: 'Дубль',
	badgeError: 'Ошибка',
	alreadyInLibrary: (title: string) => `Уже в библиотеке: ${title}`,
	reportTitle: 'Импорт завершён',
	reportAdded: (n: number) => `Добавлено книг: ${n}`,
	reportSkipped: (n: number) => `Пропущено дублей: ${n}`,
	reportErrors: (n: number) => `Строк с ошибками: ${n}`,
	reportShelves: (n: number) => `Создано полок: ${n}`,
